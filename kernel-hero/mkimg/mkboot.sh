...
ROOT_DIR_NAME=
ROOT_IMG_NAME=/root/ramdisk.img
BOOT_IMG_NAME=boot.img
KERNEL_IMAGE=../arch/arm/boot/zImage
PRJ_PATH=
CMD_LINE="no_console_suspend=1 console=null"

IMGDIR=mkimg
IMGTOOL=/opt/android-tools/mkbootimg
FSTOOL=/opt/android-tools/mkbootfs
ROOTFS=
...
if [ "${CMD_LINE}" == "" ]; then
	...
	${IMGTOOL} --kernel kernel --ramdisk ${ROOT_IMG_NAME} --cmdline "${CMD_LINE}" --output ${BOOT_IMG_NAME} --base '0x19200000'
fi
